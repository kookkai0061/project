...
Class Master extends DBConnection {
	...
	function save_blog(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `blog` set {$data} ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		}else{
			$sql = "UPDATE `blog` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				if(!is_dir(base_app.'uploads/blog_'.$id)){
					mkdir(base_app.'uploads/blog_'.$id);
					$data = " `upload_path`= 'uploads/blog_{$id}' ";
				}else{
					$data = " `upload_path`= 'uploads/blog_{$id}' ";
				}
				$this->conn->query("UPDATE `blog` set {$data} where id = '{$id}' ");
				foreach($_FILES['img']['tmp_name'] as $k =>$v){
					move_uploaded_file($_FILES['img']['tmp_name'][$k],base_app.'uploads/blog_'.$id.'/'.$_FILES['img']['name'][$k]);
				}
			}
	...
	function delete_blog(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `blog` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/blog_'.$id)){
				$file = scandir(base_app.'uploads/blog_'.$id);
				foreach($file as $img){
					if(in_array($img,array('..','.')))
						continue;
					unlink(base_app.'uploads/blog_'.$id.'/'.$img);
				}
				rmdir(base_app.'uploads/blog_'.$id);
			}
